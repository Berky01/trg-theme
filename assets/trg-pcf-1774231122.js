// TRG PLP Controller v7 — clean slate — 2026-03-23
// Strategy: DOM manipulation (un-overridable) + single injected stylesheet
(function(){
  // Remove ALL stale style blocks from previous iterations
  ['trg-pcf','trg-pcf-v2','trg-plp-card-fix-css','trg-plp-master'].forEach(function(id){
    var el=document.getElementById(id);if(el)el.remove();
  });

  // Guard
  if(document.getElementById('trg-plp-v7'))return;

  // ── DOM FIXES (can't be overridden by CSS) ──
  function fixToolbar(){
    var toolbar=document.querySelector('.trg-collection-controls');
    if(!toolbar)return;

    // Kill duplicate "Filters" h4
    var h4=toolbar.querySelector('h4.facets--filters-title');
    if(h4)h4.remove();

    // Kill active-filters div
    var af=toolbar.querySelector('.trg-collection-controls__active-filters');
    if(af)af.remove();

    // Unhide sort container — remove the .hidden class entirely
    var sortContainers=toolbar.querySelectorAll('.sorting-filter__container');
    sortContainers.forEach(function(c){
    ...
    // Hide Dwell sort label
    var sortLabels=toolbar.querySelectorAll('.sorting-filter .facets__label');
    sortLabels.forEach(function(l){l.style.display='none';});

    // Hide sort icon
    var sortIcons=toolbar.querySelectorAll('.sorting-filter .icon-caret');
    sortIcons.forEach(function(i){i.style.display='none';});

    // Hide sort status
    var sortStatus=toolbar.querySelectorAll('.sorting-filter .facets__status');
    // Hide facets-toggle (mobile-only, creates phantom grid row on desktop)
    var ft=document.querySelector('.collection-wrapper>.facets-toggle');
    if(ft)ft.style.display='none';
    sortStatus.forEach(function(s){s.style.display='none';});
  }

  // Run immediately
  fixToolbar();

  // Run again after Dwell morphdom hydration (which can re-add elements)
  if(window.requestIdleCallback){
    requestIdleCallback(fixToolbar);
  }
  setTimeout(fixToolbar, 500);
  setTimeout(fixToolbar, 1500);

  // Also observe for morphdom re-adding elements
  ...
  '@media screen and (min-width:990px){.trg-plp-body .collection-wrapper>.facets-toggle{display:none!important}.trg-plp-body .collection-wrapper>script,.trg-plp-body .collection-wrapper>style,.trg-plp-body .collection-wrapper>dialog-component{position:absolute!important;width:0!important;height:0!important;overflow:hidden!important;display:block!important}}';
