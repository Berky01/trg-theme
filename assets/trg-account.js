...
/* TRG: Force Playfair on PDP price — aggressive DOM mutation */
(function trgFixPrice() {
  function run() {
    /* Find ALL product-price elements on page */
    var allPP = document.querySelectorAll('product-price');
    allPP.forEach(function(pp) {
      /* Only target the main PDP price, not card prices */
      /* PDP price is inside .product-details or product-information */
      var isInPDP = pp.closest('.product-details') || pp.closest('.product-information');
      var isInCard = pp.closest('product-card') || pp.closest('.product-card') || pp.closest('.resource-list');
      if (!isInPDP || isInCard) return;
      
      /* Force styles on the container */
      pp.style.setProperty('display', 'block', 'important');
      pp.style.setProperty('padding-bottom', '1.25rem', 'important');
      pp.style.setProperty('border-bottom', '1px solid #ddd8cf', 'important');
      pp.style.setProperty('margin-bottom', '0.75rem', 'important');
      
      /* Force styles on ALL children */
      var children = pp.querySelectorAll('*');
      ...
      /* Also force on the element itself */
      pp.style.setProperty('font-family', "'Playfair Display', serif", 'important');
      pp.style.setProperty('font-size', '1.5rem', 'important');
      pp.style.setProperty('font-weight', '600', 'important');
      pp.style.setProperty('text-transform', 'none', 'important');
    });
  }
  ...
  /* Run after delays to catch Dwell morphdom hydration */
  setTimeout(run, 100);
  setTimeout(run, 500);
  setTimeout(run, 1500);
  setTimeout(run, 3000);
  /* Also run on any DOM change in product area */
  var observer = new MutationObserver(function() { run(); });
  var target = document.querySelector('.product-information') || document.querySelector('[data-product-grid-content]') || document.body;
  observer.observe(target, { childList: true, subtree: true });
  /* Disconnect after 10 seconds to avoid infinite loop */
