...
function init(){
  ...
  bindShopChips();
  if(dk())bindNav();
  bindTabs();bindSearch();bindBk();bindKb();bindX();bindCC();bindLR();
  render('all','');
  if(dk())watchChv();
}
...
function normalizeShopChips(){
  $$('#trg-mm-p-shop .trg-mm-acs').forEach(function(row){
    ...
    normalized.forEach(function(chip){if(chip.tagName!=='A')chip.setAttribute('aria-pressed',chip.classList.contains('on')?'true':'false')});
  });
...
function render(cat,q){
  ...
  g.innerHTML=cols.map(function(col){return'<div class="trg-mm-col">'+col.map(function(it){return'<div class="trg-mm-lt" data-letter="'+it.l+'" id="trg-mm-lt-'+it.l+'">'+it.l+'</div>'+it.b.map(function(b){return'<a href="/collections/'+b.slug+'" class="trg-mm-bl">'+esc(b.name)+'</a>'}).join('')}).join('')+'</div>'}).join('');

  renderLR(ls);

}

function renderLR(active){

  var rail=$('trg-mm-lr');if(!rail)return;

  var letters='ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('');

  var act={};(active||[]).forEach(function(l){act[l]=1});

  rail.innerHTML=letters.map(function(l){var on=act[l]?'':' off';return'<button type="button" class="trg-mm-lr-b'+on+'" data-jump="'+l+'"'+(on?' aria-disabled="true" tabindex="-1"':'')+'>'+l+'</button>'}).join('');

}

function bindLR(){

  var rail=$('trg-mm-lr');if(!rail||rail.__b)return;rail.__b=true;

  rail.addEventListener('click',function(e){

    var b=e.target.closest('.trg-mm-lr-b');if(!b||b.classList.contains('off'))return;

    var l=b.getAttribute('data-jump'),t=document.getElementById('trg-mm-lt-'+l);

    if(t&&t.scrollIntoView)t.scrollIntoView({behavior:'smooth',block:'nearest',inline:'nearest'});
...
function bindKb(){if(window.__trgMegaMenuKbBound)return;window.__trgMegaMenuKbBound=true;document.addEventListener('keydown',function(e){if(e.key==='Escape'&&op){close();return}if((e.metaKey||e.ctrlKey)&&(e.key==='k'||e.key==='K')){var tg=e.target,tn=tg&&tg.tagName;if(tn==='INPUT'||tn==='TEXTAREA'||(tg&&tg.isContentEditable))return;if(!dk())return;e.preventDefault();if(op!=='brands')toggle('brands');var i=$('trg-mm-bi');if(i)setTimeout(function(){i.focus();i.select()},140)}})}
