// TRG PLP DOM Controller — from scratch — 2026-03-23
// CSS is in trg-plp.css. This file handles DOM-only fixes.
(function(){
  ...
  function fix(){
    ...
    // 1. Remove .facets-toggle (Dwell mobile btn, phantom grid row on desktop)
    var ft=cw.querySelector(':scope > .facets-toggle');
    if(ft) ft.remove();

    // 2. Remove dialog-component from grid flow
    var dlg=cw.querySelector(':scope > dialog-component');
    if(dlg) dlg.style.cssText='display:none!important';

    // 3. Toolbar: remove duplicate h4.facets--filters-title
    var h4=cw.querySelector('.trg-collection-controls h4.facets--filters-title');
    if(h4) h4.remove();

    // 4. Toolbar: remove .trg-collection-controls__active-filters
    var af=cw.querySelector('.trg-collection-controls__active-filters');
    if(af) af.remove();

    // 5. Toolbar: unhide .sorting-filter__container (Dwell adds .hidden class)
    ...
    // 6. Toolbar: hide sort label, icon, status
  ...
  // Also run on Dwell section events
